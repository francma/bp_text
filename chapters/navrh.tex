...
Integrační proces je rozdělen na fáze (\textit{stage}), které jsou vykovány v zadaném pořadí sekvenčně za sebou.
Fáze se skládá z jednoho nebo více na sobě nezávislých úkolů (\textit{job}), které je možné paralelizovat. 
...
Soubor může být deklarativním popisem dat (JSON, YAML, XML, \ldots) nebo jednoduchý procedurální skript (Lua, Python, \ldots).
...
Nad daty systému je třeba provádět také filtrace (například vrať mi všechny integrace tohoto projektu, které byli provedeny na SCM větvi \textit{master}).
